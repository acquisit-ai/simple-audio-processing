...
用法：
  python3 5agent-mapping-deepseek.py <input_json> <output_json>
"""
...
6. 对每个输出 token：
   - 生成 token.explanation：当前语境下的额外解释、自然含义、语气、用法或短语结构说明；不要只写简单直译
   - 生成 semantic_element.base_form：单词的原形（running -> run，studies -> study), 或者短语无时态形式(looking forward to -> look forward to)
   - 生成 semantic_element.translation：简短中文直译，通常是一个或几个中文词或很短短语，不写语法、用法或完整解释
...
- "break down" -> 固定搭配
- "be addicted to" -> explanation: "在这里表示对某事上瘾；addicted 表示沉迷的、上瘾的"，translation: "上瘾", base_form: "addicted"
...
JSON 输出协议：
- 只输出一个合法 JSON object，不要输出 Markdown，不要用 ```json 代码块包裹。
- `action` 必须是 "match"、"search"、"no_match" 之一。
- `reason` 必须是非空字符串。
- `match` 输出必须包含 `coarse_id`，可以包含 `explanation`。
- `search` 输出必须包含 `queries`，最多 4 个字符串。
- `no_match` 输出不要包含 `coarse_id`。

EXAMPLE JSON OUTPUT - match:
{
  "action": "match",
  "coarse_id": 166670,
  "reason": "当前 token 在语境中表示 want 的动词义，与候选 coarse_unit 语义可靠匹配。"
}

EXAMPLE JSON OUTPUT - search:
{
  "action": "search",
  "queries": ["set up", "set"],
  "reason": "当前短语候选不足，继续查询更标准的核心表达。"
}

EXAMPLE JSON OUTPUT - no_match:
{
  "action": "no_match",
  "reason": "当前候选没有语义可靠匹配，继续搜索也不太可能得到更合适结果。"
}
"""
...
class DeepSeekJsonLLM:
    """通过 DeepSeek OpenAI-compatible API 请求 JSON mode 输出。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to .env containing DEEPSEEK_API_KEY and DEEPSEEK_BASE_URL.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        help="DeepSeek model for stage one.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="DeepSeek model for stage three.",
    )
    parser.add_argument(
...
def load_deepseek_config(env_path: Path) -> tuple[str, str]:
    """从 `.env` 读取 DeepSeek API Key 和 base URL。"""
...
def create_deepseek_json_llm(
    env_path: Path,
    model_name: str,
    reasoning_effort: str,
) -> DeepSeekJsonLLM:
    """创建一个 DeepSeek JSON mode 聊天模型。"""

    api_key, base_url = load_deepseek_config(env_path)
    client = OpenAI(api_key=api_key, base_url=base_url)
    return DeepSeekJsonLLM(client, model_name, reasoning_effort)


def parse_llm_json_response(content: str, schema: type[BaseModel]) -> BaseModel:
    """解析 DeepSeek JSON mode 文本，并用 Pydantic schema 做后置校验。"""

    stripped = content.strip()
    if not stripped:
        raise ValueError("DeepSeek returned empty content")
    if stripped.startswith("```"):
        raise ValueError("DeepSeek returned Markdown fenced content instead of a raw JSON object")
...
def invoke_stage_one(
    ...
    messages: list[dict[str, str]] = [
        ...
        {
            "role": "user",
            "content": "请按上述规则输出 json。输入批次 JSON：\n"
            + json.dumps(cleaned_batch, ensure_ascii=False, indent=2),
        },
    ]
    result = parse_llm_json_response(stage_one_llm.invoke_json(messages), StageOneOutput)
...
def build_stage_three_messages(
    ...
    messages: list[dict[str, str]] = [
        {"role": "system", "content": STAGE_THREE_SYSTEM_PROMPT},
        {"role": "user", "content": STAGE_THREE_RULES_PROMPT},
        {"role": "user", "content": build_current_round_instruction(rounds[-1].round_no)},
        {
            "role": "user",
...
def invoke_stage_three_decision(
    ...
    result = parse_llm_json_response(stage_three_llm.invoke_json(messages), StageThreeDecision)
...
def finalize_match(
    ...
    - translation <- candidate.chinese_label
    - dictionary <- candidate.chinese_def
    - kind <- candidate.kind
    - pos <- candidate.pos, if present
    ...
    semantic_element["translation"] = matched.get("chinese_label") or semantic_element.get("translation", "")
    semantic_element["dictionary"] = matched.get("chinese_def") or semantic_element.get("dictionary", "")
    semantic_element["kind"] = matched["kind"]
    if matched.get("pos"):
        semantic_element["pos"] = matched["pos"]
    else:
        semantic_element.pop("pos", None)
...
def finalize_no_match(
    ...
    按文档要求：
    - coarse_id 设为 null
    - 第一阶段的 base_form、translation 和 dictionary 保持不变
    - 不写入 kind / pos
    - reason 仍然必须写入
    """
