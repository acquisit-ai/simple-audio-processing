...
用法：
  python3 5agent-mapping-gemini.py <input_json> <output_json>
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Gemini model for stage one.",
    )
    parser.add_argument(
        "--stage-one-thinking-level",
        default=DEFAULT_STAGE_ONE_THINKING_LEVEL,
        choices=sorted(ALLOWED_GEMINI_THINKING_LEVELS),
        help="Gemini thinking level for stage one.",
    )
    parser.add_argument(
        "--stage-three-model",
        default=DEFAULT_STAGE_THREE_MODEL,
        help="Gemini model for stage three.",
    )
    parser.add_argument(
        "--stage-three-thinking-level",
        default=DEFAULT_STAGE_THREE_THINKING_LEVEL,
        choices=sorted(ALLOWED_GEMINI_THINKING_LEVELS),
        help="Gemini thinking level for stage three.",
    )
...
def create_gemini_json_llm(
    ...
    """创建一个绑定结构化输出 schema 的 Gemini 模型。"""

    load_dotenv(env_path)
    normalized_thinking_level = thinking_level.lower()
    return GeminiJsonLLM(client, model_name, normalized_thinking_level, schema)
...
def invoke_stage_one(
    ...
    messages: list[dict[str, str]] = [
        ...
        {"role": "user", "content": json.dumps(cleaned_batch, ensure_ascii=False, indent=2)},
    ]
...
def main() -> None:
    ...
    stage_one_llm = create_gemini_json_llm(
        client=gemini_client,
        env_path=args.env_path,
        model_name=args.stage_one_model,
        thinking_level=args.stage_one_thinking_level,
        schema=StageOneOutput,
    )
    stage_three_llm = create_gemini_json_llm(
        client=gemini_client,
        env_path=args.env_path,
        model_name=args.stage_three_model,
        thinking_level=args.stage_three_thinking_level,
    ...
    log_step(f"阶段 1 thinking_level：{args.stage_one_thinking_level}")
    log_step(f"阶段 3 模型：{args.stage_three_model}")
    log_step(f"阶段 3 thinking_level：{args.stage_three_thinking_level}")
