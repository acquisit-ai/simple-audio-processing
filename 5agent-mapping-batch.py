#!/usr/bin/env python3
"""
批量调用 5agent-mapping.py 处理 transcript JSON。
...
- 最多开启 5 个并发任务
- 如果目标目录已存在同名文件，则直接跳过
- 屏蔽子进程 `5agent-mapping.py` 的命令行输出
"""
...
MAPPING_SCRIPT = ROOT_DIR / "5agent-mapping.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch run 5agent-mapping.py over transcript JSON files.")
    parser.add_argument(
        "--source-dir",
        type=Path,
