...
```text
你将看到第二阶段输出的 token 结果，以及数据库查询返回的 coarse_unit 候选。

你的任务是判断某个 token 是否可以可靠映射到某个 coarse_unit。

规则：

1. 只能在提供的候选中选择，不允许生成不存在的 coarse_id。
2. 必须以语义一致为第一标准，而不是只看字符串相似。
3. 若多个候选都可能匹配，选择语义最贴合当前上下文的一个。
4. 若没有足够把握，则返回不匹配。
5. 当前处理对象由 `sentence.index + token.index` 唯一确定，你只能判断当前这个 token，不要处理同句中的其他 token。
6. 搜索流程最多 3 回：
   - 第 1 回：自动执行一次 `exact` 搜索，同时传入 `token.text` 和 `baseForm`
   - 第 2 回：若仍未匹配，则允许 agent 在 `exact` 模式下自由尝试最多 4 个相似查询词
   - 第 3 回：若仍未匹配，则允许 agent 在 `contain` 模式下自由尝试最多 4 个相似查询词
7. 三回搜索中任意时点一旦匹配成功，立即停止该 token 的搜索。
8. 同一个 token 的三回搜索可以持续 append 当前 token 的对话上下文；但切换到下一个 token 时，必须回到第二阶段产物作为根上下文，不能继承上一个 token 的搜索对话。
9. 若匹配成功：
   - 只返回你确认的 coarse_id 与匹配 reason
   - 不要自行写入最终的 baseForm 和 dictionary
   - 脚本会在校验 coarse_id 合法后，用数据库真值替换 baseForm 和 dictionary
   - explanation 可以给出建议版本
10. 若三回结束仍未匹配成功：
   - coarse_id 设为 null
   - 其他字段保持第一阶段原值
11. 输出结构由系统提供的 structured output schema 约束，不要自行改变字段结构。
```
...
- `baseForm` 和 `dictionary` 的最终值不由 LLM 提供，而由脚本根据数据库候选真实值回填
