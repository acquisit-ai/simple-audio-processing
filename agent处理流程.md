      ...
      "tokens": [
        {
          ...
          "semantic_element": {
            "coarse_id": 166670,
            "base_form": "want",
            "translation": "想要/要求/寻求",
            "dictionary": "表达主观的想要、请求或寻求某人/某物（有意图/行为性）。包括对物品的渴望、要求/让某人做事，以及因目的而寻求某人。",
            "kind": "word",
            "pos": "verb",
...
- `coarse_id` 允许为 `null`
- `translation` 表示简短中文直译或数据库中文标签
- `dictionary` 表示最终保留的稳定释义文本
- `kind` / `pos` 只在数据库匹配成功后出现；如果数据库 `pos` 为空，则不写 `pos`
- `semantic_element.reason` 表示第三阶段对该 token 的匹配或失败原因，必须为非空字符串
- 正式输出中的句子必须带 `start` / `end`
- 正式输出中的 token 必须带 `start` / `end`
- 如果数据库成功匹配并判定语义一致，则 `translation` 使用数据库返回的 `chinese_label`
- 如果数据库成功匹配并判定语义一致，则 `dictionary` 使用数据库返回的 `chinese_def`
- 如果数据库没有可靠匹配，则保留第一阶段生成的 `translation` 和词典释义，且不写 `kind` / `pos`
...
- 若流程中断，可通过 `temp/` 中间文件里的最后一个 `sentence.index` 判断已完成到哪里
- 续跑时应从最后一个已成功保存的 `sentence.index` 之后继续
- 第三阶段必须直接使用第二阶段产出的 JSON 作为新输入开启新对话
- 第三阶段不继续 append 第一阶段的对话上下文
- 正式输出文件只有在全部句子都成功处理后才会写入目标目录

### 中间文件与正式输出规则

每次批次保存时，不直接写目标输出文件，而是写入目标目录下 `temp/` 中的中间文件。

要求：

- 每个批次成功后，将当前累计结果写入 `temp/<目标输出文件名>`
- 中间文件必须采用原子替换方式写入，不能直接覆盖
- 不允许在批次处理中途把未完整跑完的结果写入正式输出文件
- 只有当所有句子都成功处理完成后，才允许生成正式输出文件
...
- 最终累计输出句子数量必须等于原始输入句子数量
- 只有数量完全一致，才允许将最终结果写入目标输出路径
- 如果数量不一致，则视为整个任务失败
- 失败时不得写入正式输出文件

正式输出写入前：

- 在第 26 步句子数量校验通过后，代码必须根据原始输入 JSON 回填正式输出中的时间信息
- 句子的 `start` / `end` 直接沿用原始输入句子的时间
- token 的 `start` / `end` 通过最终 token 文本重新顺序对齐原始 token 列表后写入
- 如果某个句子里某个 token 的时间对齐失败，则当前句子中该 token 以及其后的 token 都写入空时间
- 这一步只在正式输出阶段执行，中间文件不写入这些时间字段

正式输出写入成功后：

- 删除 `temp/` 中与当前目标输出文件对应的中间文件
- `temp/` 目录本身不删除
...
- 搜索审计记录文件保存在目标输出文件所在目录下的 `log/` 子目录中
...
5. 合并目标以英语学习为导向：
   ...
   - 普通自由组合不要强行合并
6. 对每个输出 token：
   - 生成 token.explanation：当前语境下的额外解释、自然含义、语气、用法或短语结构说明
   - 生成 semantic_element.base_form
   - 生成 semantic_element.translation：简短中文直译，通常是一个中文词或很短短语，不写语法、用法或完整解释
   - 生成 semantic_element.dictionary：上下文无关词典释义，可以比 translation 更完整，但不要展开当前句子的语境
...
- 若两次重试后仍失败，则整个流程直接失败
- 当前失败批次不进入阶段 3
- 当前失败批次不写入正式输出文件，也不更新 `temp/` 中间文件
...
- `CoarseQueryRunner`
  ...
  - 处理 query 去重与结果解析
- `AuditLogger`
- 将每个 token 的搜索过程追加写入 `log/<原始输入 JSON 文件名>.search_audit.jsonl`
...
- `index`
- `text`
- `semantic_element.base_form`
- `semantic_element.translation`
- `explanation`
- `semantic_element.dictionary`

### 搜索策略

第三阶段允许 agent 在后续轮次自由发挥搜索词，但总流程仍然固定，最多只搜索 3 回，并且三回都只使用 `exact` 搜索。

每次只处理一个 token，并且一次对话最多只确认这一个 token 的匹配结果。

在进入数据库查询与 LLM 判断之前，脚本还应先执行一层固定前置判断：

- 维护一个固定词表，用于存放“过于简单、极高频、低学习价值、继续搜索意义不大”的基础功能词
- 判断依据使用：
  - `token.text`
  - `semantic_element.base_form`
- 两者都先做归一化：
  ...
  - 去掉首尾标点
- 如果任意一个命中该词表，则：
  - 该 token 直接按 `no_match` 处理
  - 不执行数据库查询
  - 不调用第三阶段 LLM
  - 直接写固定 `reason`
  - 记录审计日志后进入下一个 token

这条规则是脚本前置逻辑，不依赖 LLM 自行判断。

词表建议至少覆盖：

- 冠词：`a`、`an`、`the`
- 基础连词：`and`、`or`、`but`、`if`
- 系动词/助动词：`is`、`are`、`was`、`were`、`be`
- 常见代词：`he`、`his`、`him`、`she`、`her`、`they`、`their`

并可继续扩展到其他同层级基础功能词。

固定 `reason` 建议写法：

- `当前 token 属于过于简单、极高频、低学习价值的基础功能词或代词，不进入 coarse_unit 映射，直接按 no_match 处理。`

这里的“1 回搜索”定义为：调用一次 `query_coarse_units.py`。

由于脚本一次最多支持 4 个查询参数，所以：

- 第 1 回虽然会同时查询 `token.text` 和 `base_form`，但只算 1 回搜索
- 第 2 回最多传入 4 个 `exact` 查询词，算 1 回搜索
- 第 3 回最多传入 4 个 `exact` 查询词，算 1 回搜索
...
- 最多执行 4 个相似查询词
- 这些查询词可基于 `token.text`、`base_form`、时态还原、短语核心词、近似表达等自由调整
- 只要任意一个查询词命中并确认匹配成功，就立即停止该 token 的搜索，进入结果回填
- 若最多 4 个 exact 相似查询词仍未匹配成功，则进入第 3 回

#### 第 3 回：agent 自由发挥，exact 模式

如果前两回都未匹配成功，则允许 agent 再自由决定一组搜索词，并继续使用 `exact` 模式。
...
- `label` 与 token 文本或 base_form 的匹配度
- `kind`
- `pos`
- `chinese_def`
- `chinese_criteria`
...
- 若校验成功：
  - `semantic_element.coarse_id = 命中的 id`
  - 用数据库返回的 `label` 替换 `semantic_element.base_form`
  - 用数据库返回的 `chinese_label` 替换 `semantic_element.translation`
  - 用数据库返回的 `chinese_def` 替换 `semantic_element.dictionary`
  - 用数据库返回的 `kind` 写入 `semantic_element.kind`
  - 如果数据库返回的 `pos` 非空，则写入 `semantic_element.pos`；如果为空，则不写 `pos`
  - 将匹配原因写入 `semantic_element.reason`
...
- `semantic_element.coarse_id = null`
- 保留第一阶段生成的 `explanation`
- 保留第一阶段生成的 `translation`
- 保留第一阶段生成的 `dictionary`
- 不写入 `semantic_element.kind` / `semantic_element.pos`
- `semantic_element.reason` 必须写入明确的失败原因
...
- 对大部分单词，第一次精确搜索就已经足够；有就是有，没有就是没有，这种情况通常不应再返回 `search`
- 只有少数词组或变形表达，才适合继续搜索更短、更核心或更标准的精确查询词
- 新查询词应尽量朝“核心词 / 更标准写法 / 去掉不必要修饰”收缩，而不是保留整段原短语继续绕圈搜索
- 不要把整句里的额外修饰词一起带入搜索；例如像 `subtle sexuality` 这种情况下，通常只需要查 `sexuality`
- 合理例子：
  - `be addicted to` -> 可以继续查 `addicted to`、`addicted`、`addict`
  - `looking forward to` -> 可以继续查 `look forward to`、`look forward`
- 不合理例子：
  - `sexuality` 第一次精确搜索没有结果时，通常就应直接 `no_match`，不应继续扩展成别的词
  - `economic` 第一次精确搜索没有结果时，通常也应直接 `no_match`
- `search.queries`
...
- 第 2 回和第 3 回的 `search` 都只表示“继续 exact 搜索”
- 若返回结构不符合当前轮次规则，则视为非法输出
...
- 将失败原因写入 `semantic_element.reason`，且不能为空
...
- `match`
- 或 `search`
- 若当前 token 属于固定词表未覆盖、但 agent 仍判断继续搜索意义不大，也允许返回 `no_match`
...
- `match`
- 或 `search`
- `search` 仍只表示“继续 exact 搜索”
- 若 agent 已判断继续搜索意义不大，也允许返回 `no_match`

### 第 3 回调用设计

如果第 2 回仍未匹配成功，则 agent 可以返回 `search`，要求控制器执行第 3 回 exact 搜索。
...
- 你会看到当前 token
- 你会看到当前句子
- 你会看到系统已经自动执行的第 1 回搜索结果：`exact(token.text, base_form)`
- 这一轮允许返回：
  - `match`
  - `search`
- 若当前 token 过于简单、极高频、低学习价值，或你判断继续搜索意义不大，也可以直接返回：
  - `no_match`
...
- 这一轮允许返回：
  - `match`
  - `search`
- 如果返回 `search`，则表示下一回仍继续做精确匹配
- 如果你已经判断继续搜索意义不大，也可以直接返回：
  - `no_match`
...
- 保留第一阶段的 `dictionary`
- 不写入 `semantic_element.kind` / `semantic_element.pos`
- `semantic_element.reason` 写入明确的校验失败原因，且不能为空

### 推荐控制器判定逻辑

1. 控制器先对当前 token 做固定词表前置判断
2. 若命中固定词表，则直接按 `no_match` 处理，写固定 `reason`，并结束该 token
3. 若未命中固定词表，控制器执行当前轮次搜索
4. 控制器把当前轮次结果附加到当前 token 分支上下文
5. agent 返回 `match` / `search` / `no_match`
6. 控制器校验返回格式是否合法
7. 若为 `match`，先校验 `coarse_id` 是否真实存在于当前数据库候选中
8. 校验通过后，再由脚本回填 `base_form` / `translation` / `dictionary` / `kind` / `pos` / `reason` 并结束该 token
9. 若 `coarse_id` 校验失败，则重试一次当前轮次 agent 调用；仍失败则按 `no_match` 处理
10. 若为 `search`，进入下一回
11. 若为 `no_match`，结束该 token 并写入 `coarse_id = null`

### 推荐控制器实现骨架

```python
def process_token(token_runtime, shared_context_A):
    if hit_direct_no_match_list(token_runtime.text, token_runtime.base_form):
        return finalize_no_match(
            token_runtime,
            "当前 token 属于过于简单、极高频、低学习价值的基础功能词或代词，不进入 coarse_unit 映射，直接按 no_match 处理。",
        )
    ...
    if decision.action == "match":
        ...
        if decision.action == "search":
            r3_candidates = run_query("exact", decision.queries)
            rounds.append({"round_no": 3, "mode": "exact", "queries": decision.queries, "candidates": r3_candidates})
            decision = ask_llm(shared_context_A, token_runtime, rounds)
...
每次批次成功完成后，都要把“当前累计结果”写入 `temp/` 中间文件。

只有当所有批次都成功完成，且最终句子数量与输入句子数量完全一致时，才允许写入正式输出文件。
...
13. 在进入数据库查询前，先对当前 token 做固定词表前置判断
14. 若命中固定词表，则直接按 `no_match` 处理，写固定 `reason`，记录审计日志，并进入下一个 token
15. 若未命中固定词表，则对当前 token 执行第 1 回默认自动 exact 搜索
    这一步是一次脚本调用，同时传入 `token.text` 和 `base_form`
16. 若未匹配成功，则在当前 token 分支上下文中进入第 2 回，由 agent 在 exact 模式下自由尝试最多 4 个查询词
17. 若仍未匹配成功，则在当前 token 分支上下文中进入第 3 回，由 agent 再自由尝试最多 4 个 exact 查询词
18. 将候选交给第三阶段 agent 做语义匹配，并在匹配成功时立即停止该 token 的搜索
19. 若 agent 返回 `match`，先校验 `coarse_id` 是否真实存在于当前数据库候选中；若非法则重试一次，仍非法则按失败流程处理
20. 校验成功后，由脚本回填当前批次的 `coarse_id` / `base_form` / `translation` / `dictionary` / `kind` / `pos` / `reason` / `explanation`
21. 将当前 token 的搜索审计记录写入目标输出目录下的 `log/` 子目录
22. 当前 token 完成后，丢弃该 token 的分支上下文，回到共享根上下文 `A`
23. 继续处理当前批次的下一个 token
24. 当前批次全部 token 完成后，将结果合并到累计结果
25. 保存当前累计 JSON 到 `temp/<目标输出文件名>` 中间文件
26. 继续下一个批次，直到全部完成
27. 所有批次完成后，校验最终句子数量是否与输入一致
28. 若一致，则根据原始输入为正式输出回填句子与 token 的 `start` / `end`
29. 写入正式输出文件，并删除 `temp/` 中对应的中间文件
30. 若句子数量不一致，则视为失败，不写正式输出文件
31. 继续下一个批次，直到全部完成
...
```bash
python3 supabase/query_coarse_units.py exact apple pear "be addicted to"
```
