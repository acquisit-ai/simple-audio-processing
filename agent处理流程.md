...
6. 对每个输出 token：
   ...
   - 生成 semantic_element.dictionary
...
8. 校验通过后，再由脚本回填 `base_form` / `dictionary` / `reason` 并结束该 token
...
20. 校验成功后，由脚本回填当前批次的 `coarse_id` / `base_form` / `dictionary` / `reason` / `explanation`
